...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:base="http://services.tmobile.com/base" version="2.0"
	...
	<xsl:variable name="varRequest" select="/searchCriteria"/>
		...
		<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
			...
			<soapenv:Body>
				<v1:getCartDetailsRequest
					...
					<xsl:if test="$varRequest">
						<xsl:if test="$varRequest/customerId">
							<xsl:element name="v1:customerId">
								<xsl:value-of select="$varRequest/customerId"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/cartId">
							<xsl:element name="v1:cartId">
								<xsl:value-of select="$varRequest/cartId"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/emailAddress">
							<xsl:element name="v1:emailAddress">
								<xsl:value-of select="$varRequest/emailAddress"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/phoneNumber">
							<xsl:element name="v1:phoneNumber">
								<xsl:value-of select="$varRequest/phoneNumber"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/changeOfResponsibility">
							<xsl:element name="v1:changeOfResponsibility">
								<xsl:if test="$varRequest/changeOfResponsibility/receiverEmailAddress">
									<xsl:element name="v1:receiverEmailAddress">
										<xsl:value-of select="$varRequest/changeOfResponsibility/receiverEmailAddress"/>
									</xsl:element>
								</xsl:if>
								<xsl:if test="$varRequest/changeOfResponsibility/receiverMSISDN">
									<xsl:element name="v1:receiverMSISDN">
										<xsl:value-of select="$varRequest/changeOfResponsibility/receiverMSISDN"/>
									</xsl:element>
								</xsl:if>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/userId">
							<xsl:element name="v1:userId">
								<xsl:value-of select="$varRequest/userId"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/dealerCode">
							<xsl:element name="v1:dealerCode">
								<xsl:value-of select="$varRequest/dealerCode"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/firstName">
							<xsl:element name="v1:firstName">
								<xsl:value-of select="$varRequest/firstName"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/lastName">
							<xsl:element name="v1:lastName">
								<xsl:value-of select="$varRequest/lastName"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/cartDate">
							<xsl:element name="v1:cartDate">
								<xsl:value-of select="$varRequest/cartDate"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/taxId">
							<xsl:element name="v1:taxId">
								<xsl:value-of select="$varRequest/taxId"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/SSN">
							<xsl:element name="v1:SSN">
								<xsl:value-of select="$varRequest/SSN"/>
							</xsl:element>
						</xsl:if>
						<xsl:if test="$varRequest/financialAccountNumber">
							<xsl:element name="v1:financialAccountNumber">
								<xsl:value-of select="$varRequest/financialAccountNumber"/>
							</xsl:element>
						</xsl:if>
					</xsl:if>
				</v1:getCartDetailsRequest>
