<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:variable name="varStatus" select="/status/cart/status/@statusCode"/>
		<apiError>
			<statusCode>
			...
			<errors>
				<xsl:choose>
					...
					<xsl:when test="string-length($varStatus) = 0">
						<error>
							<code>
								<xsl:value-of select="&quot;GENERAL-0001&quot;"/>
							</code>
							...
							<systemMessage>
								<xsl:value-of select="&quot;statuscode is missing&quot;"/>
							</systemMessage>
						</error>
					</xsl:when>
					<xsl:when test="not(contains($varStatus, 'Saved') or contains($varStatus, 'Reviewed')or contains($varStatus, 'Cancelled')or contains($varStatus, 'CheckedOut'))">
						<error>
							<code>
								<xsl:value-of select="&quot;GENERAL-0001&quot;"/>
							</code>
							...
							<systemMessage>
								<xsl:value-of select="&quot;Invalid statuscode&quot;"/>
							</systemMessage>
						</error>
					</xsl:when>
