...
var jsonResponse = context.getVariable("payLoad").replace(/"~STR~"/, "null").replace(/~STR~/g, "").replace(/"~ARRAY~",/g, "").replace(/,"~ARRAY~"/g, "").replace(/"~ARRAY~"/g, "").replace(/"~INT~~INT~"/g, "null").replace(/"~INT~/g, "").replace(/~INT~"/g, "");

var resp = JSON.parse(jsonResponse).response;
if(resp != null) {
  	context.setVariable("payLoad",JSON.stringify(resp));
} else{
  	context.setVariable("payLoad",jsonResponse);
}
